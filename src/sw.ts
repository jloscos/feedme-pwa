...
self.addEventListener("install", event => {
    event.waitUntil(caches.open(cacheKey.static).then(cache => {
        cache.addAll([
            "/",
            "/favicon.ico",
            "/index.html",
...
async function respondToFetch(event: FetchEvent) {
    ...
    if (m) {
        await IndexDBHelper.setValue<CachedArticle>("cachedArticle", { articleId: +m[1], cacheKey: "article" + cacheVersion + "_" + m[1], cacheDate: new Date() });
    }
    return response;
}
